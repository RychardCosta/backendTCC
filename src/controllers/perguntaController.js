  ...
  async gerar(req, res) {
    ...
    if (verificarPerguntasRepetidas === "true" && alunoId) {
      ...
      try {
        ...
        const searchPerguntasFiltradas = await connection("Pergunta").select("*").whereNotIn("id", arrayId).where({ categoriaID })

        if (searchPerguntasFiltradas.length === 0) {
          const perguntas = await connection("Pergunta").select("*").where({ professorId, categoriaID });
